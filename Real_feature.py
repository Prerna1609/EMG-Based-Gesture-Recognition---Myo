...
import pandas as pd  # Added for waveform length function
...
# This is the waveform length feature extraction function you provided.
def wave_form_length(raw_data):
    rows = []
    size = 50
    slices = int(raw_data.shape[0] / size)
    for sample in range(0, slices):
...
def emg_handler(emg_values, movement):
    ...
    if len(emg_window) == window_size:
        ...
        # Compute waveform length feature:
        # Convert the deque to a DataFrame (with columns for each EMG channel)
        raw_df = pd.DataFrame(np.array(emg_window), columns=['ch1','ch2','ch3','ch4','ch5','ch6','ch7','ch8'])
        wl_features = wave_form_length(raw_df)
        # Since our window size equals the function's window (50 samples), there will be only one row.
        print("Waveform Length Features:", wl_features.iloc[0].values)

# Initialize and connect Myo
